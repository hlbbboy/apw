...
async def cookie2user(cookie_str):
    ...
    try:
        ...
        if sha1 != hashlib.sha1(s.encode('utf-8')).hexdigest():
            ...
            return None
        user.passwd = '******' # 这里对passwd的更改没有提交到数据库，只是出于密码安全考虑，删掉内存中的密码
        return user
...
async def signout(request):
    ...
    r.set_cookie(COOKIE_NAME, '-deleted-', max_age=0, httponly=True) # 可以直接用'del_cookie'
...
async def api_register_user(*, email, name, passwd):
    ...
    users = await User.findAll('email=?', [email])
    if len(users) > 0:
        raise APIError('register:failed', 'email', 'email ia already in use.')
    uid = next_id()
...
async def api_blogs(*, page='1'):
    page_index = get_page_index(page)
    num = await Blog.findNumber('count(id)')# 'count(id)' select count(*) 函数,返回括号内查找到的数量
    p = Page(num, page_index)
    if num == 0:
